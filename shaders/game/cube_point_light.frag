...
in Varyings
{
    ...
    vec3 viewNormal;
    vec2 texCoord;
    vec4 cubePointLightPosition;
} In;

uniform sampler2D _texture;
uniform samplerCube _cubePointLightTexture;
...
uniform CubePointLightBlock
{
    vec4 viewPosition;
    mat4 viewToLightMatrix;
} CubePointLight;
...
vec3 calcCubePointLightLighting(vec3 diffuseColor)
{
    Light light;
    light.viewPosition = CubePointLight.viewPosition;
    light.intensity = texture(_cubePointLightTexture, In.cubePointLightPosition.xyz) * 4.0;
...
void main()
{
    ...
    accumulatedLight += calcCubePointLightLighting(diffuseColor);
