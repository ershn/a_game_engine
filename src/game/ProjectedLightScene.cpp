...
struct SpotlightShader : public Gfx::Shader
{
    Gfx::UniformBlock light_block{};
    Gfx::UniformBlock spotlight_block{};
    Gfx::SamplerUniform sampler{};
    Gfx::SamplerUniform spotlight_sampler{};

    SpotlightShader(GLuint shader_program)
        : Gfx::Shader{shader_program, {.lv_normal_matrix = true}}
        , light_block{Gfx::OGL::get_uniform_block_index(shader_program, "LightBlock")}
        , spotlight_block{Gfx::OGL::get_uniform_block_index(shader_program, "SpotlightBlock")}
        , sampler{Gfx::OGL::get_uniform_location(shader_program, "_texture")}
        , spotlight_sampler{Gfx::OGL::get_uniform_location(shader_program, "_spotlightTexture")}
    {
    }
};

struct SpotlightMaterial : public Gfx::Material
{
    ...
    {
        auto &shader = static_cast<SpotlightShader &>(this->shader);
        bind_uniform_buffer_range(shader.shader_program, shader.light_block, light_buffer_range_id);
        bind_uniform_buffer_range(shader.shader_program, shader.spotlight_block, spotlight_buffer_range_id);
        bind_texture_and_sampler(shader.sampler, texture_id, sampler_id);
        bind_texture_and_sampler(shader.spotlight_sampler, spotlight_texture_id, spotlight_sampler_id);
    }
};
...
struct Spotlight
{
    static constexpr Core::ComponentType TYPE{SPOTLIGHT};

    Core::EntityId camera_id{};
    float vertical_fov{};
    Gfx::UniformBuffer<SpotlightBlock> uniform_buffer;
    Gfx::UniformBufferRangeId uniform_buffer_range_id{};
};

Math::Matrix4 projective_texturing_matrix(float vertical_fov)
{
    return Math::translation_matrix({0.5f, 0.5f, 0.0f}) * Math::affine_scaling_matrix({0.5f, 0.5f, 1.0f}) *
           Math::perspective_proj_matrix(1.0f, vertical_fov);
}

void calc_spotlight_matrix(Spotlight &spotlight, const Core::Transform &transform)
{
...
void ProjectedLightScene::init() const
{
    ...
    Gfx::TextureId spotlight_texture_id{next_texture_id++};
    {
        Gfx::TextureData texture_data{};
        if (!Gfx::read_texture_data_from_dds_file("assets/game/textures/bands_of_light.dds", texture_data))
            return;

        Gfx::load_texture(spotlight_texture_id, texture_data);
    }
    ...
    Gfx::SamplerId spotlight_sampler_id{next_sampler_id++};
    Gfx::create_sampler(
        spotlight_sampler_id,
        Gfx::SamplerParams{.flags{
            .texture_wrap_s = Gfx::TextureWrapMode::CLAMP_TO_BORDER,
            .texture_wrap_t = Gfx::TextureWrapMode::CLAMP_TO_BORDER
        }}
    );
    ...
    auto directional_light_id = Core::create_entity(
        ...
        Gfx::DirectionalLight{.light_intensity{0.2f, 0.2f, 0.2f, 1.0f}}
    );

    // Point light
    auto point_light_id = Core::create_entity(
        Core::Transform{.position{5.0f, 6.0f, 0.5f}},
        Gfx::PointLight{.light_intensity{Math::Vector4{3.5f, 6.5f, 3.0f, 1.0f} * 0.5f}}
    );

    auto spotlight_buffer = Gfx::create_uniform_buffer<SpotlightBlock>();
    auto spotlight_buffer_range_id = spotlight_buffer.create_range();
    ...
    auto spotlight_id = Core::create_entity(
        ...
        Spotlight{
            .camera_id = camera_id,
            .vertical_fov = 60.0f,
            .uniform_buffer = spotlight_buffer,
            .uniform_buffer_range_id = spotlight_buffer_range_id
        }
    );
    ...
    // Floor
    {
        ...
        material.spotlight_texture_id = spotlight_texture_id;
        material.spotlight_sampler_id = spotlight_sampler_id;
...
void ProjectedLightScene::update() const
{
    ...
    process_components(calc_spotlight_matrix);
