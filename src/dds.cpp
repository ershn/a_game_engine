...
)
{
    ...
    if (header.ddspf.flags & DDPF_FOURCC) // block compressed or custom texture format
    {
        ...
        {
        case DDS_DXT1:
            texture_desc.format = ImageFormat::BC1_UNORM;
            break;
        case DDS_DXT2:
            texture_desc.format = ImageFormat::BC2_UNORM;
        ...
        case DDS_DXT3:
            ...
            break;
        case DDS_DXT4:
            texture_desc.format = ImageFormat::BC3_UNORM;
            texture_desc.alpha_type = AlphaType::PREMULTIPLIED;
            break;
        case DDS_DXT5:
            texture_desc.format = ImageFormat::BC3_UNORM;
            ...
            break;
        case DDS_DX10:
            texture_desc.format = static_cast<ImageFormat>(header_extension.dxgiFormat);
            break;
        }
    }
    else
            ...
            else
            {
                if (header.ddspf.rgbBitCount == 8)
                {
                    if (header.ddspf.rBitMask == 0xff)
                        texture_desc.format = ImageFormat::R8_UNORM;
                }
            }
        }
    ...
    VBAIL_ERROR_IF(texture_desc.format == ImageFormat::UNKNOWN, false, "unrecognized texture format: {}", file_path);
    return true;
}
